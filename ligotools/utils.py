...
# function to generate plots in the given cell
def plot_results(det, time, timemax, tevent, SNR, strain_whitenbp,
                       template_match, template_fft, datafreq, freqs, data_psd,
                       d_eff, fs, fig_dir, eventname, plottype, make_plots=True):
    if make_plots:
        if det == 'L1':
            pcolor = 'g'
        else:
        ...
        plt.xlabel('Time since {0:.4f}'.format(timemax))
        plt.legend(loc='upper left')
        plt.savefig(os.path.join(fig_dir, eventname+"_"+det+"_SNR."+plottype))
        ...
        plt.savefig(os.path.join(fig_dir, eventname+"_"+det+"_matchtime."+plottype))
        ...
        plt.savefig(os.path.join(fig_dir, eventname+"_"+det+"_matchfreq."+plottype))
